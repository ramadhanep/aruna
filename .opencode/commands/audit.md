...
Both are read-only for this command — none should edit source code or docs/ content beyond writing their own report file.

After both finish:
1. Summarize each report's top 5 findings by severity/effort.
2. Note any overlap between the two reports.
3. Recommend whether `/plan` should run next, or whether docs should be refreshed first if needed.
