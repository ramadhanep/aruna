...
5. Follow `CLAUDE.md`'s "When Implementation Changes" rules as you go: update the relevant `docs/*.md` for any architectural change, update `docs/environment.md` for new env vars, update `docs/api.md` for new/changed routes, update `docs/database.md` and `supabase/setup.sql` together for schema changes, update `docs/dependencies.md` for new packages.
...
8. Update `docs/ai-session-handoff.md` per `CLAUDE.md`'s rule to keep session continuity current.

Never touch `public/sw.js` without bumping `VERSION` and checking the cache-strategy doc, per `CLAUDE.md`'s explicit "Never" rule. Never skip the plan-mode confirmation step, even if the phase looks small.
