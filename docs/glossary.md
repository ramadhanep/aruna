...
| **Vanta.js** | JavaScript library for animated 3D website backgrounds (used on old landing page). |
| **Yield Sign (🪣)** | Visual indicator in the codebase for "bucket" (storage bucket reference in comments). |
| **Butter-Smooth** | A code comment pattern emphasizing smooth scrolling performance (appears in mobile bottom nav). |
