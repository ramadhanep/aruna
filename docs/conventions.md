...
| `sidebar_state` | Sidebar collapsed/expanded (cookie via shadcn sidebar, not localStorage) |
...
- Tiny type scale: `text-2xs` (10px), `text-1xs` (11px) and `text-3xs` (9px)
  theme tokens replace `text-[10px]`/`text-[11px]`/`text-[9px]` — use them for dense
  metadata; don't introduce new arbitrary font sizes.
...
- Images use `next/image` (`<Image>`). `next.config.mjs` sets
  `images.unoptimized: true`, so URLs pass through unchanged (keeps `/aruna.png`
  service-worker precache intact and avoids Vercel image-optimization usage); no
  `remotePatterns` are needed for avatars.
...
- Formatters: `formatPrice`, `formatMarketCap`, `formatPercent`, `formatCompactNumber`, `formatDecimalPercent`, `formatUSD`, `formatIDR`, `formatSGD`, `formatByCurrency`, `getChangeTone` in `lib/utils.js`.
