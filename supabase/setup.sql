-- =============================================================================
-- Aruna – Complete Database Setup
-- Run this file once against a fresh Supabase project to create all tables,
-- RLS policies, indexes, seed data, and migrations.
-- =============================================================================
...
-- Money Flow Analyzer tables
create table if not exists public.money_flow_reports (
  id bigint generated by default as identity primary key,
  symbol text not null,
  report_date date not null,
  money_flow_score numeric(6, 2) not null,
  broker_accdist text not null,
  top1_percent numeric(10, 4) not null default 0,
  top3_percent numeric(10, 4) not null default 0,
  top5_percent numeric(10, 4) not null default 0,
  total_buyer integer not null default 0,
  total_seller integer not null default 0,
  value numeric(20, 2) not null default 0,
  volume numeric(20, 2) not null default 0,
  today_volume numeric(20, 2) not null default 0,
  avg_volume_20 numeric(20, 2) not null default 0,
  volume_spike numeric(12, 4) not null default 0,
  current_price numeric(15, 2) not null default 0,
  price_1m_ago numeric(15, 2) not null default 0,
  price_change_1m numeric(12, 6) not null default 0,
  price_change_5d numeric(10, 2) not null default 0,
  price_change_10d numeric(10, 2) not null default 0,
  price_change_20d numeric(10, 2) not null default 0,
  price_range_10d numeric(10, 2) not null default 0,
  signal text not null,
  score_breakdown jsonb not null default '{}'::jsonb,
  broker_summary jsonb not null default '[]'::jsonb,
  broker_inventory jsonb not null default '[]'::jsonb,
  broker_cost_analysis jsonb not null default '{}'::jsonb,
  broker_concentration jsonb not null default '{}'::jsonb,
  absorption_strength jsonb not null default '{}'::jsonb,
  accumulation_persistence jsonb not null default '{}'::jsonb,
  market_phase text not null default 'Indeterminate',
  manipulation_risk jsonb not null default '{"level":"LOW","reasons":[]}'::jsonb,
  analysis_summary text,
  screener_id bigint,
  screener_name text,
  screener_snapshot jsonb not null default '{}'::jsonb,
  created_at timestamptz default now(),
...
create table if not exists public.weekly_reports (
  ...
  top_picks jsonb not null default '[]'::jsonb,
  source_count integer not null default 0,
  min_score numeric(6, 2),
  screener_id bigint,
  screener_name text,
  screener_total_rows integer not null default 0,
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz default now()
);

alter table public.money_flow_reports enable row level security;
alter table public.weekly_reports enable row level security;

create policy "Anyone can view money flow reports" on public.money_flow_reports
  for select using (true);

create policy "Anyone can view weekly money flow picks" on public.weekly_reports
  for select using (true);

create policy "Service role maintains money flow reports" on public.money_flow_reports
  for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');

create policy "Service role maintains weekly money flow picks" on public.weekly_reports
  for all using (auth.role() = 'service_role') with check (auth.role() = 'service_role');

create index if not exists money_flow_reports_report_date_idx on public.money_flow_reports (report_date desc);
create index if not exists money_flow_reports_symbol_date_idx on public.money_flow_reports (symbol, report_date desc);
create index if not exists money_flow_reports_score_idx on public.money_flow_reports (money_flow_score desc);
create index if not exists money_flow_reports_market_phase_idx on public.money_flow_reports (market_phase);
